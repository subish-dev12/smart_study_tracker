        ...
        <div className="p-6 bg-slate-900/5 dark:bg-slate-800/60 rounded-2xl border border-slate-700/20 shadow-md">
          <div className="flex gap-4 items-center">
            ...
            <form onSubmit={handleForm} className="flex gap-2">
              <input
                name="task"
                className="border border-slate-700 bg-transparent text-slate-900 dark:text-slate-100 placeholder-slate-500 dark:placeholder-slate-400 rounded-lg px-3 py-2 shadow-sm focus:ring-2 focus:ring-indigo-400/30 caret-slate-900 dark:caret-slate-100"
                value={addInput}
          ...
          <ul className="mt-6 space-y-3">
            {task.map((item) => (
              <li
                key={item.id}
                className={`flex justify-between items-center p-4 rounded-2xl border border-slate-700/20 bg-linear-to-r from-slate-800/10 to-slate-700/10 shadow-md hover:scale-[1.01] transform-gpu transition-all duration-200 border-l-4 border-l-transparent ${item.completed ? " bg-green-400" : ""}`}
              >
                {editMode === item.id ? (
                  <>
                    <input
